jQuery(function ($) {
    ...
    $('#register-delete-confirm').click(function(){
        $(this).blur();
        centeringModalSyncer();
        if($("#modal-overlay")[0]) return false; // 既にmodalが表示されていたら何も返さない
        $("body").append('<div id="modal-overlay"></div>'); // 背景を追加する
    ...
    $(".register-delete").on('click',function(){
        console.log("OK");
    ...
    function centeringModalSyncer(){
        ...
        //コンテンツ(#modal-content)の幅を取得し、変数[cw]に格納
        var cw = $("#modal-content-delete").outerWidth();

        //コンテンツ(#modal-content)の高さを取得し、変数[ch]に格納
        var ch = $("#modal-content-delete").outerHeight();
        ...
        //[#modal-content]のCSSに[left]の値(pxleft)を設定
        $("#modal-content-delete").css({'left': pxleft});
    }
